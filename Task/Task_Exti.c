...
#include "stdio.h"
#include "string.h"
WaterMeter_t WaterMeterList[16] =
{ 0 };
...
void TaskFunc_Exti(void *argument)
{
	...
	for (;;)
	{
		...
		if (++count % 10 == 0)
		{
			for (uint8_t i = 0; i < 16; i++)
			{
				printf("[%hu]Count:%lu\tFreq:%lu\t\n", i, WaterMeterList[i].pulseCnt, WaterMeterList[i].pulseFreq);
			}
		}
...
uint32_t WaterMeter_GetCount(WaterMeter_t *wm)
{
	return wm->pulseCnt;
}

uint32_t WaterMeter_GetFreq(WaterMeter_t *wm)
{
	return wm->pulseFreq;
}
