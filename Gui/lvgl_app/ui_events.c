...
void refreshUserBlock(UserBlock_t* ub) {
    char buf[16];
    int32_t bar_value;
    bar_value = WaterMeterList[ub->wmIndex].pulseFreq / 100;
    lv_bar_set_value(ub->bar_speed, bar_value, LV_ANIM_ON);
    bar_value =
        (WaterMeterList[ub->wmIndex].pulseLimit > 0)
            ? (100 * WaterMeterList[ub->wmIndex].pulseCnt / WaterMeterList[ub->wmIndex].pulseLimit)
            : 100;
    lv_bar_set_value(ub->bar_total, bar_value, LV_ANIM_ON);

    sprintf(buf, "%.2fL/Min", WaterMeterList[ub->wmIndex].pulseFreq * LperPULSE);
    lv_label_set_text(ub->lb_speed, buf);
    sprintf(buf, "%.2fL", WaterMeterList[ub->wmIndex].pulseCnt * LperPULSE);
    lv_label_set_text(ub->lb_total, buf);
...
//    printf("selected_btn:%u\n", selected_btn);
    ...
    } else if (keyMode == LV_KEYBOARD_MODE_TEXT_UPPER && selected_btn == 35) {
